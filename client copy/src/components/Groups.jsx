...
export default function Groups({ members, pastGroups, setPastGroups }) {
  ...
  const generateShuffledGroups = (array) => {
    ...
    while (!uniqueGroupFound) {
      ...
      const groupExists = pastGroups && pastGroups.some((pastGroup) => {
        return isGroupEqual(pastGroup, groupedArray);
      });
    ...
    // Update pastGroups only with unique groups
   
    console.log("groupedArray:",groupedArray, "pastGroups: ", pastGroups)
  ...
  useEffect(() => {
    if (members && members.length >= 12) {
      ...
      console.log(shuffledGroups, pastGroups)
      setGroups(shuffledGroups);
      setPastGroups(prevPastGroups => [shuffledGroups, ...prevPastGroups]);
    }
